...
const queryYouBikeApi = async () => {
  ...
  const startingStation = collection.find((station) => station.sno === config.startingStationId);
  const targetStation = collection.find((station) => station.sno === config.targetStationId);
  const secondStation = collection.find((station) => station.sno === config.secondStationId);
  return {
    updatedAt,
    startingStation,
    targetStation,
